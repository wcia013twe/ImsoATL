/**
 * Fetch Atlanta boundary from OpenStreetMap using Overpass API
 *
 * This script:
 * 1. Queries Overpass API for Atlanta city boundary
 * 2. Converts OSM data to GeoJSON
 * 3. Calculates bounding box and center
 * 4. Saves to /app/frontend/public/data/cities/atlanta.json
 */
...
// Overpass QL query for Atlanta city boundary
// Searches for the relation tagged as the City of Atlanta, Georgia
// Using bounding box for Georgia to narrow search
const OVERPASS_QUERY = `
[out:json][timeout:25];
(
  relation["boundary"="administrative"]["name"="Atlanta"]["admin_level"="8"](33.5,-84.6,34.0,-84.2);
);
out geom;
`;
...
console.log('🚀 Fetching Atlanta boundary from OpenStreetMap...\n');
...
function osmToGeoJSON(osmData) {
  ...
  if (!osmData.elements || osmData.elements.length === 0) {
    throw new Error('No boundary data found for Atlanta');
  }
  ...
  console.log(`   ✓ Found relation: ${relation.tags?.name || 'Atlanta'}\n`);
  ...
  return {
    type: 'Feature',
    properties: {
      name: relation.tags?.name || 'Atlanta',
      state: 'Georgia',
      osmId: relation.id,
      osmType: 'relation'
    },
...
async function main() {
  try {
    ...
    console.log('✅ Atlanta boundary fetch complete!');
    console.log('\nNext steps:');
